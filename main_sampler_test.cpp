...
//values based off spectrum graph of RBG values
//range from 0 -> 1
double SpectrumValues(double range, double *Rout, double *Gout, double *Bout){
	if(range <= 0.165){
		*Rout = 1;
		*Gout = range * 6.06;//eq of line
		*Bout = 0;
	}
	if(range > 0.165 && range <= 0.33){
		*Rout = (range * -6.06) + 2;
		*Gout = 1;
		*Bout = 0;
	}
	if(range > 0.33 && range <= 0.5){
		*Rout = 0;
		*Gout = 1;
		*Bout = (range * 5.88) - 1.94;
	}
	if(range > 0.5 && range <= 0.66){
		*Rout = 0;
		*Gout = (range * -6.25) + 4.13;
		*Bout = 1;
	}
	if(range > 0.66 && range <= 0.825){
		*Rout = (range * 6.06) - 4;
		*Gout = 0;
		*Bout = 1;
	}
	if(range > 0.825){
		*Rout = 1;
		*Gout = 0;
		*Bout = (range * -6.06) + 6.06;
	}
}

void SpectrumDisplay(XadcCore *adc_p, PwmCore *pwm_p, SsegCore *sseg_p){
	/*****************************
	**ADC input to select color
	 ******************************/
	//read channel 0 voltage
	double volt = adc_p->read_adc_in(0);

	/*****************************
	**PWM output to show color
	 ******************************/
	//50Hz frequency since dealing with LEDS
	pwm_p->set_freq(50);

	//input duty cycles values from function
	double Rduty,Gduty,Bduty;
	SpectrumValues(volt, &Rduty, &Gduty, &Bduty);
	pwm_p->set_duty(Bduty,0);
	pwm_p->set_duty(Gduty,1);
	pwm_p->set_duty(Rduty,2);
	pwm_p->set_duty(Bduty,3);
	pwm_p->set_duty(Gduty,4);
	pwm_p->set_duty(Rduty,5);
	sleep_ms(100);//reduce noise
	...
	/*****************************
	 **SSEG output to show voltage
	 ******************************/
	//split voltage
	int volt0 = (((int)(volt * 1000)) % 100) % 10;
	int volt1 = ((int)(volt * 100)) % 10;
	int volt2 = (int)(volt * 10);

	//write voltage
	sseg_p->set_dp(0x08);
	sseg_p->write_1ptn(sseg_p->h2s(volt0), 0);
	sseg_p->write_1ptn(sseg_p->h2s(volt1), 1);
	sseg_p->write_1ptn(sseg_p->h2s(volt2), 2);
	sseg_p->write_1ptn(sseg_p->h2s(0), 3);
}
